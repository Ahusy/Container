...
import code.ytn.cn.home.adapter.NewInputAdapter;
 ...
 * 新建出库单
 */
public class NewInputActivity extends ToolBarActivity implements View.OnClickListener, NewInputAdapter.IsONClick {

    @ViewInject(R.id.new_input_drk)
    private TextView newDrk;
    @ViewInject(R.id.new_input_dck)
    private TextView newDck;
    @ViewInject(R.id.new_input_lv)
    private ListView lv;
    @ViewInject(R.id.new_input_choose)
    private TextView newChoose;
    @ViewInject(R.id.new_input_commit)
    private TextView newCommit;
    @ViewInject(R.id.left_button)
    private ImageView left;
    private String cabinetId;
    private String cabinetName;
    private List<ChooseGoodsListEntity> goodsList = new ArrayList<>();
    private String mGoods;

    public static void startNewInputActivity(Context context,String cabinetId,String cabinetName,String depotName){
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.new_input_choose: // 选择
                startActivityForResult(new Intent(this, ChooseGoodsActivity.class).putExtra(CABINET_ID, cabinetId).putExtra(EXTRA_FROM, NEWINPUT), 1);
                break;
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == 1 && resultCode == 4) {
            ...
            if (goodsList != null && goodsList.size() > 0) {
                NewInputAdapter adapter = new NewInputAdapter(this, goodsList);
    ...
    private void tojson() {
        ...
        for (int i = 0; i < goodsList.size(); i++) {
            ...
            try {
                ...
                tmpObj.put("input_num", goodsList.get(i).getInputNum());
    ...
    private void HideKeyBoard() {
        ((InputMethodManager) getSystemService(INPUT_METHOD_SERVICE)).hideSoftInputFromWindow(NewInputActivity.this.getCurrentFocus()
    ...
    private void commit() {
        ...
        requestHttpData(Constants.Urls.URL_POST_NODEPOT_IN_STORAGE, REQUEST_NET_ONE, FProtocol.HttpMethod.POST, params);
    }
