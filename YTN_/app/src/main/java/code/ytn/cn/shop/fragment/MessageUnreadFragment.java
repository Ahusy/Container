...
public class MessageUnreadFragment extends BaseFragment {
            ...
            case REQUEST_NET_ONE:
                if (messageEntity != null) {
                    ...
                    for (int i = 0; i < listEntities.size(); i++) {
                        if (listEntities.get(i).getMsgCode().equals("1")){
                    ...
                    lv.setAdapter(new CommonAdapter<MessageEntity.MessageListEntity>(getActivity(), newMessageList, R.layout.item_frag_message) {
                        ...
                        public void convert(CommonViewHolder holder, MessageEntity.MessageListEntity messageListEntity, int position, View convertView) {
                            ...
                            holder.getView(R.id.btn_zd).setVisibility(messageListEntity.getMsgCode().equals("2") ? View.GONE : View.VISIBLE);
                            ...
                            try {
                                @SuppressLint("SimpleDateFormat") SimpleDateFormat simpleDateFormat = new SimpleDateFormat("yyyy-MM-dd HH:m:s");
                            ...
                            holder.setOnClickListener(R.id.btn_zd, v -> {
                                ...
                                // 标记已读
                                signRead(messageListEntity.getMsgId());
                                messageListEntity.setMsgCode("2");
                                notifyDataSetChanged();
                                Intent intent = new Intent("msg");
                                intent.putExtra("change", "msg_uRead");
    ...
    // 标记已读
    private void signRead(String id) {
        IdentityHashMap<String, String> params = new IdentityHashMap<>();
        params.put("id", id);
        requestHttpData(Constants.Urls.URL_POST_MESSAGE_DETAIL, REQUEST_NET_TWO, FProtocol.HttpMethod.POST, params);
    }
    ...
    private void deleteMsg(String id) {
        IdentityHashMap<String, String> params = new IdentityHashMap<>();
        params.put("id", id);
        requestHttpData(Constants.Urls.URL_POST_DELETE_MESSAGE, REQUEST_NET_THREE, FProtocol.HttpMethod.POST, params);
    }
    ...
    private BroadcastReceiver mAdDownLoadReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if ("msg_read".equals(change) || "msg_all".equals(change)){
